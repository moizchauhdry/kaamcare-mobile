...
export const useMutationPutADL = (
  ...
  return useMutation<void, ErrorResponse, ADLSections, { previousList: ADLSections }>({
    ...options,
    mutationKey: [QUERY_KEYS.GOALS_OF_CARE_ADL_PUT],
    mutationFn: (values) => adlClient.putADLData(values),
    ...
    onSuccess: () => {
      showToast({
        text1: `ADL's has been successfully updated.`,
      });
    },
